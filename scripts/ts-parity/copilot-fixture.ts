...
async function captureCopilotRequest(): Promise<CapturedRequest> {
	...
	if (!captured) {
		throw new Error("TS provider did not issue a Copilot request");
	}
	return captured;
}
...
async function main() {
	...
	if (!outputDir) {
		throw new Error("usage: copilot-fixture.ts <output-dir>");
	}
	await mkdir(outputDir, { recursive: true });
	await writeFile(
		join(outputDir, "github-copilot-gpt-5.4.json"),
		`${JSON.stringify(
			{
				source: {
					branch: "ts-reference",
				...
				auth: "api-key",
				request: await captureCopilotRequest(),
			},
			null,
			2,
		)}\n`,
